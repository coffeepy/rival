...
async function slowStep({ input }: StepContext) {
	const delayMs = (input as { delayMs?: number })?.delayMs ?? 50;
...
async function testWaitOptions() {
	console.log("\n--- TEST: wait() options (event + polling modes) ---\n");

	const workflow = createWorkflow("waitOptions").step(slowStep).build();
	const engine = rival(workflow);

	// Polling-only path
	const runIdPolling = `wait-polling-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
	await engine.run("waitOptions", { delayMs: 30 }, runIdPolling);
	const pollingResult = await engine.wait("waitOptions", runIdPolling, {
		preferEvents: false,
		pollIntervalMs: 10,
		timeoutMs: 2_000,
	});
	assert(pollingResult.status === "completed", "wait() polling-only mode completes");

	// Event-first path with very short event wait timeout, should still succeed via fallback
	const runIdEvent = `wait-event-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
	await engine.run("waitOptions", { delayMs: 30 }, runIdEvent);
	const eventResult = await engine.wait("waitOptions", runIdEvent, {
		preferEvents: true,
		eventWaitTimeoutMs: 1,
		pollIntervalMs: 10,
		timeoutMs: 2_000,
	});
	assert(eventResult.status === "completed", "wait() event-first mode completes with fallback");
}
